...
class MainLLM:
    ...
    def _build_system_instruction(self, context: Dict[str, Any]) -> str:
        ...
        parts.append(
            "\n[도구 사용 규칙]\n"
            "도구를 적극적으로 활용하세요. 최신 정보, 번역 등이 필요하면 주저하지 말고 도구를 사용하세요.\n"
            "도구(검색, 기억 조회 등)를 호출할 때 **빈말이나 중간 과정을 절대 출력하지 마세요.** "
            "'잠시만 기다려 주세요', '찾아보겠습니다', '검색해볼게요' 같은 멘트를 절대 하지 마세요. "
            "도구는 조용히 실행하고, 결과를 받은 뒤 **최종 답변만** 자연스럽게 전달하세요."
        )
    ...
    def _prepare_gemini_request(self, user_input, system_instruction, l1_recent, context):
        ...
        user_info = (
            ...
            f"[핵심 지침] **사용자의 마지막 발화(가장 최근 메시지)**에 집중해서 답변하세요. "
            f"이전 대화의 미해결된 요청보다 **현재의 요청**을 최우선으로 처리하세요."
        )
